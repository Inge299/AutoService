...
SMS.RU подключается через `SMS_PROVIDER=smsru`; его credentials и имя отправителя
задаются только через environment. Отчёты и транскрибация ещё не реализованы.
