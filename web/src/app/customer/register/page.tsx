...
export default async function CustomerRegisterPage({ searchParams }: {
  ...
  if (!approval) return <main className="login-page"><section className="login-panel"><div className="login-form-wrap"><BrandMark /><h2>Нужна персональная ссылка</h2><p className="muted login-subtitle">Откройте ссылку согласования, которую прислала мастерская, и создайте кабинет из неё.</p><ActionLink variant="secondary"  href="/customer/login">У меня уже есть кабинет</ActionLink></div></section></main>;
  ...
  return <main className="login-page">
    ...
    <section className="login-panel"><div className="login-form-wrap">
      ...
      {mode === "call" && challenge && dialPhone ? <form action={verifyCustomerRegistrationCallAction} className="login-form">
        ...
        <ActionLink variant="secondary"  href={`/customer/register?approval=${encodeURIComponent(approval)}`}>Запросить заново</ActionLink>
        ...
        <Button variant="primary" className="" type="submit">Создать и перейти в кабинет</Button>
        <ActionLink variant="secondary"  href={`/customer/register?approval=${encodeURIComponent(approval)}`}>Запросить подтверждение заново</ActionLink>
      </form> : <form action={requestCustomerRegistrationCodeAction} className="login-form">
        <input type="hidden" name="approval" value={approval} />
        <Button variant="primary" className="" type="submit">Подтвердить звонком</Button>
      </form>}
      ...
      <ActionLink variant="secondary"  href={`/customer/login?approval=${encodeURIComponent(approval)}`}>У меня уже есть кабинет</ActionLink>
    </div></section>
  </main>;
}
