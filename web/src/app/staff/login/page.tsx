...
export const metadata: Metadata = { title: "Вход сотрудника" };
...
const errors: Record<string, string> = {
  ...
  sms_unavailable: "Вход по телефону временно недоступен.",
...
export default async function StaffLoginPage({ searchParams }: { searchParams: Promise<{ error?: string; mode?: string; challenge?: string; callPhone?: string; callPhonePretty?: string }> }) {
  const { error, mode, challenge, callPhone, callPhonePretty } = await searchParams;
  const dialPhone = callPhone?.replace(/[^\d+]/g, "");
  return <main className="login-page">
    <section className="login-story">
      <Link href="/staff/login" className="login-brand"><BrandMark /></Link>
      <div className="login-message"><span className="eyebrow light">Рабочее пространство мастерской</span><h1>Весь ремонт —<br />в понятном процессе.</h1><p>Визиты, доказательства, решения клиентов и повторные обращения в одном спокойном интерфейсе.</p></div>
    ...
    <section className="login-panel"><div className="login-form-wrap">
      ...
      <h2>{mode === "call" ? "Позвоните для входа" : mode === "code" ? "Введите код" : "Вход в AutoService"}</h2>
      <p className="muted login-subtitle">{mode === "call" ? "Позвоните с вашего рабочего номера. Звонок будет сброшен автоматически." : mode === "code" ? "Введите код из SMS." : mode === "sms" ? "Подтвердите рабочий номер звонком." : "Войдите по паролю или подтвердите рабочий номер звонком."}</p>
