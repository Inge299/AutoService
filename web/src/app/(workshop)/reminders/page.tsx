...
export default async function RemindersPage() {
  ...
  return (
    <>
      ...
      <section className="summary-banner"><span className="summary-icon"><Icon name="reminders" /></span><div><strong>{urgentCount ? `${urgentCount} требуют внимания` : "Срочных напоминаний нет"}</strong><p>{reminders.length ? `Всего запланировано: ${reminders.length}.` : "Сервер пока не содержит запланированных контактов."}</p></div><button className="button button-primary" disabled title="Станет доступно после подключения SMS endpoint">Подготовить отправку</button></section>
