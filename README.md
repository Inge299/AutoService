...
- для согласования мастер сначала ждёт подтверждения хотя бы одного материала сервером, затем создаёт одноразовую публичную ссылку и передаёт её через системное меню Android; решение принимает только клиент на публичной странице;
